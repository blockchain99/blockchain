...
public class Transaction {
	...
	//A transaction input consists of the hash of the transaction that 
	//contains the corresponding output, the index of this output in that transaction 
	...
	//over the current transaction with the public key in the spent output. These facts are implemented in 
	//following getRawDataToSign(int index),This file, verifySignature()in Crypto.java .
	/*
	 * Input is the receiving of BTC, and an Output is a sending of BTC.
	 * Output must have been received by an input of an earlier date. Transactions keep track of 
	 * the Output and the Input. UTXO(Unspent Transaction Output) means output,
	 * Whenever you send an amount of money to another address you utilize 
	 * past transactions (inputs) send to your address.
	 * 
	 * These inputs are never actually destroyed when they reach your address, 
	 * this means that your total Bitcoins is dependent on the sum of all independent inputs to your address.
	 */
    public class Input {
        ...
        public Input(byte[] prevHash, int index) {
            if (prevHash == null)  //if previous block not exit=this is starting points.
                prevTxHash = null;
            else
...
//A transaction output consists of a value and a public key to which it is being paid
    ...
    public void removeInput(UTXO ut) {
        for (int i = 0; i < inputs.size(); i++) {
            ...
            if (u.equals(ut)) {  //if each element of "inputs",ArrayList equals with given unspent transaction obj(output)
                inputs.remove(i);
                return;
            }
        }
...
//More specifically, the raw data that is signed is obtained from the getRawDataToSign(int index) method.
    public byte[] getRawDataToSign(int index) {
        ...
        if (index > inputs.size())
            return null;
        Input in = inputs.get(index);  //return value of ArrayList,"inputs" with specific index.
